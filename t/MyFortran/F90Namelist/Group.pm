...
#  under Darcs now]
#
# Modified by Alex Harvey to allow for the use of multiple namelists with the 
# same name. 2022

package MyFortran::F90Namelist::Group;

=head1 NAME

MyFortran::F90Namelist::Group - Parse F90 namelist groups and export in different formats
...
Extract or merge namelists from group and return I<MyFortran::F90Namelist> object:
...
I<MyFortran::F90Namelist::Group> is a module for parsing Fortran90 namelist
groups into an internal format based on
L<MyFortran::F90Namelist|MyFortran::F90Namelist>, and re-exporting in different
formats.
Parsing is done by L<MyFortran::F90Namelist|MyFortran::F90Namelist>, see the
...
L<MyFortran::F90Namelist|MyFortran::F90Namelist> object.
...
Return the first namelist in the group as
L<MyFortran::F90Namelist|MyFortran::F90Namelist> object and remove it from the
group.
...
is another way of doing
  ...
  my $nl = MyFortran::F90Namelist->new();
...
sub new {
    ...
    my $short_usage =
        "Usage:\n" .
        "  MyFortran::F90Namelist::Group->new()\n" ;
    #    unless($file) {
    #	croak $short_usage;
    #	return undef;
    #    }
...
#
    ...
    if ($text eq '') {
	croak "MyFortran::F90Namelist::Group->parse(): need text or file argument\n"
	    unless ($file ne '');
	local $/ = undef;
	...
	$nl->parse($text);
        ...
        if (defined($name) and $name ne '') {
            if ($name eq "wfngrp"){    # Alex - Modified to store repeated namelist
                if ($self->{DATA}->{$name}){
                    push(@{$self->{DATA}->{$name}}, $nl);
                } else {
                    $self->{DATA}->{$name} = [$nl];
                }
            } else {
...
#
    ...
    croak "Usage:  MyFortran::F90Namelist::Group::insert(\$nlobj,\$pos)\n"
      unless (ref($nlobj) eq "MyFortran::F90Namelist");
...
#
    ...
    my $nlname;
    if (ref($arg1) eq 'MyFortran::F90Namelist') { # F::NL object
	$nlname = $arg1->name();
...
sub first {
#
# Return the first namelist in the group as MyFortran::F90Namelist object.
#
    my $self = shift();
...
sub nth {
#
# Return the n-th namelist in the group as MyFortran::F90Namelist object.
# Counts from 0.
#
    my $self = shift();
    my $n    = shift();
    ...
    if (($n < 0) || ($n >= $nlists)) {
        croak "MyFortran::F90Namelist::Group->nth(): "
...
sub pop {
#
# Return the first namelist in the group as MyFortran::F90Namelist object (or
...
sub flatten {
#
# Merge all namelist data in the group into one MyFortran::F90Namelist object.
#
    my $self = shift();
...
# Optional arguments (see MyFortran::F90Namelist.pm):
