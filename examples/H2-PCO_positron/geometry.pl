...
%geometry = (
  ...
  'geometry_labels',  "   R    ",         # labels used on the first line of output files
...
for (my $rrr = 1.4;  $rrr <= 1.41; $rrr += 0.4) {

  push(@{$geometry{'geometries'}},
     { 'description', sprintf("%8.5f", $rrr),
       'gnuplot_desc', "R = ".sprintf("%5.2f", $rrr), # used in gnuplot files for keys
       'natype', 2,
