...
%model = ( 
  ...
  'molecule',    "H2+",              # Used only for directory and descriptions
  'atoms',  ["H", "H"],              # All atoms must be specified
  'nelectrons',      1,              # number of target electrons
  'symmetry',    "D2h",              # Options are "D2h", "C2v", "C2", "Cs", "C2h", "D2", "Ci", "C1"
  ...
  'orbitals',           "HF",         # which orbitals to use, HF = Hartree-Fock orbitals (default), natural = natural orbitals obtained by Molpro with CASSCF
  'charge_of',  "scattering",         # PHOTO/RMT: use orbitals for N-electron system (= "target") or (N+1)-electron system (= "scattering")
  'select_orb_by',  "molden",         # UKRmol+ only: molden = use orbitals as ordered in Molden file, energy = use orbitals ordered by energy
  ...
  'nactive',       1,                 # number of active target orbitals (used for SEP and CAS)
                                      #   if -1 then all available orbitals provided by the basis are used 
  'nvirtual',      0,                 # number of virtual orbitals
  'nreference',   16,                 # number of orbitals used for searching reference orbitals
  ...
  'active_orbs',   [1,0,0,0,0,0,0,0], # which orbitals for each symmetry to use as active, 
  'virtual_orbs',  [0,0,0,0,0,0,0,0], # which orbitals for each symmetry to use as virtual, 
  'reference_orbs',[2,2,2,2,2,2,2,2], # which orbitals for each symmetry to use for searching reference orbitals, 
  ...
  'ncasscf_states', {'singlet', [0,0,0,0,0,0,0,0],
  ...
  'ntarget_states', {'doublet', [1,0,0,0,0,0,0,0],  # number of target states to calculate in each irreducible representation (IR)
  ...
  'scattering_states', {'singlet', [1,1,1,1,1,1,1,1], # 1/0 to run/not to run scattering calculation for a given spin-symmetry
                        'triplet', [0,0,0,0,0,0,0,0]},
  ...
  'start_BTO',         0.0,              # radius where the B-splines start
  'order_BTO',           9,              # order of the B-splines
  'no_of_BTO',          15,              # number of B-splines
  'maxl_BTO',            4,              # the highest partial wave used in the continuum B-spline basis
  'maxl_legendre_1el',  14,              # maximum L to use in Legendre expansion for nuclear attraction integrals in the B-sline basis
  'maxl_legendre_2el',  14,              # maximum L to use in Legendre expansion for 2-electron integrals in the B-sline basis
 ...
 # Propagation step
  ...
  'raf',              10.0,              # radius at which continued fraction method can be used for R-matrix propagation

 # Energy grid
  'nescat',          "600",              # number of input scattering energies in each subrange (input for R_SOLVE via namelist &rslvin)
  'einc',     "0.05, 0.05",              # scattering energies - initial energy, energy increment, there can be more subranges
 ...
 # Initial and final states for which the cross sections are calculated
  ...
  'first_Ip',         15.4,              # first ionization potential (used by dipelm only) - units set by e_unit
