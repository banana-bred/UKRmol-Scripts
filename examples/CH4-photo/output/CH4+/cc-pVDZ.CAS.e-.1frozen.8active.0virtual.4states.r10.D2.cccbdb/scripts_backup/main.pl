...
# by Karel Houfek from Charles University in Prague
#  (karel.houfek@mff.cuni.cz, or karel.houfek@gmail.com)
...
my %data = ( # this hash array together with option hash arrays above will be passed to subroutines calling programs
  ...
  'orbitals',   {'energies',    {}, # energies of molecular orbitals as hash array, e.g. "2.1" -> -1.23456 is energy of the 1st state with symmetry 2
                 'target_all',  [], # from target sword or Molpro: number of all target MOs in each irreducible representation (IR)
                 'target_sum',   0, # from target sword or Molpro: sum of all target MOs from each irreducible representation (IR)
...
foreach my $file ($0, @ARGV) { # $0 is the main script
  system("$cp_cmd $file $dirs{'scripts'}${bs}$file");
}
foreach my $file (@{$run{'add_files_to_backup'}}) { # Copy the libraries
  system("$cp_cmd $INC{$file} $dirs{'scripts'}${bs}$file");
}
...
foreach $r_geom (@{$data{'geometries'}}) {
  ...
  if ($data{'igeom'} >= $geometry{'start_at_geometry'} &&
    ...
    if ($run{'ukrmolplus'} == 1 && $run{'remove_moints'} == 1) {
