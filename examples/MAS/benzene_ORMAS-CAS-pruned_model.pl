...
%model = ( 
  ...
  'molecule',    "benzene",              # Used only for directory and descriptions
  'atoms',        ["C", "C", "H", "H", "C", "C", "H", "H", "C", "C", "H", "H"],   # All atoms must be specified
  ...
  'charge_of',      "target",         # use orbitals for N-electron system (= "target") or (N+1)-electron system (= "scattering")
  'select_orb_by',  "molden",         # molden = use orbitals as ordered in Molden file, energy = use orbitals ordered by energy
  'model',          "ORMAS-CAS-pruned",         # options are "SE"    = static-exchange, 
                                      #             "SEP"   = SE + polarization, 
  ...
  'nfrozen',       19,                 # number of frozen target orbitals (used for SEP and CAS)      
  'nactive',       9,                 # number of active target orbitals (used for SEP and CAS)
                                      #   if -1 then all available orbitals provided by the basis are used 
  'nvirtual',      0,                 # number of virtual orbitals
  'nreference',   28,                 # number of orbitals used for searching reference orbitals
  ...
  'frozen_orbs',   [6,1,4,0,5,0,3,0], # which orbitals for each symmetry to use as frozen, 
  'active_orbs',   [2,1,1,1,1,1,1,1], # which orbitals for each symmetry to use as active, 
  'virtual_orbs',  [0,0,0,0,0,0,0,0], # which orbitals for each symmetry to use as virtual, 
  'reference_orbs',[8,2,5,1,6,1,4,1], # which orbitals for each symmetry to use for searching reference orbitals, 

  # The following are used for the Multiple Active Spaces (MAS) approach.
  # ---------------------------------------------------------------------
  ...
  # These options overide the setting of orbital spaces above, except for the total number of orbitals per
  ...
  # The format for defining the space is [subspace], [min_occ, max_occ], /(frozen|closed|active)/
  ...
  # deletion thresholds, used in swedmos or scatci_integrals
  ...
  # number of target states used for orbital optimisation in MOLPRO CASSCF calculation
  'ncasscf_states', {'singlet', [2,1,1,1,1,1,1,1],
                     'triplet', [1,1,1,1,1,1,1,1]},
  # number of all target states which will be calculated
  'ntarget_states', {'singlet', [2,1,1,1,1,1,1,1],  # number of target states to calculate in each irreducible representation (IR)
                     'triplet', [1,1,1,1,1,1,1,1]},
  ...
  'scattering_states', {'doublet', [1,1,1,1,1,1,1,1], # 1/0 to run/not to run scattering calculation for a given spin-symmetry
  ...
  'raf',              50.0,              # radius at which continued fraction method can be used for R-matrix propagation
